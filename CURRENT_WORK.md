...
Последний commit с изменениями кода: `344fde4`
...
- Исправлена прокрутка повестки на Qt 6: используются реальные `contentHeight`, `availableWidth`/`availableHeight` и effective-размеры полос прокрутки; позиция всегда ограничивается допустимым диапазоном.
- Контекстное меню таймера переведено на общий Qt 6-компонент меню; динамические пункты больше не накапливаются при повторных открытиях.
...
- Автоматический Plasma runtime-тест прокрутки повестки — успешно: длинный список прокручивается до вычисленного максимума, после очистки короткий список остаётся на `contentY = 0`.
- Живой запуск после унификации меню таймера — без QML/runtime errors в затронутом компоненте.
- Визуальная проверка в текущей Plasma-сессии — компактное и полное представления, календарь, повестка, таймер и его контекстное меню используют согласованную тёмную палитру. Более светлая верхняя полоса на снимке относится к декорации окна `plasmawindowed`, а не к плазмоиду; резкий светло-тёмный контраст предыдущего временного стенда был вызван пустым `XDG_CONFIG_HOME`.
...
Локальные `master` и `dev` указывают на чистую базу `b2f7fd7` (`origin/master`). Все изменения этой итерации перенесены в `ID-003`; изменения кода зафиксированы до commit `344fde4`. Удалённые ветки не изменялись. Пользовательские изменения не откатывались; временные runtime-стенды, снимки и `__pycache__` удалены.
